...
public:
  unsigned char ipNtm;
  unsigned char stsPmult;
  unsigned char stsMnth;
};
...
class CIA_ADJUST {
public:
  unsigned short pdoaAdj;
};

// 0x0F – Digital diagnostics interface
// Should be defined

// 0x11 – PMSC control and status

// 0x12 – RX frame buffer 0

// 0x13 – RX frame buffer 1

// 0x14 – Transmit data buffer

// 0x15 – Accumulator CIR memory

// 0x16 – Scratch RAM

// 0x17 – AES KEY RAM

// 0x18 – Double buffer diagnostic register set

// 0x1D – Indirect pointer A

// 0x1E – Indirect pointer B

// 0x1F – FINT status and indirect pointer interface
...
public:
  ...
  SAR_CTRL *sarCtrl = new SAR_CTRL();
  SAR_STATUS *sarStatus = new SAR_STATUS();
  SAR_READING *sarReading = new SAR_READING();
  SAR_WAKE_RD *sarWakeRd = new SAR_WAKE_RD();
  PGC_CTRL *pgcCtrl = new PGC_CTRL();
  PGC_STATUS *pgcStatus = new PGC_STATUS();
  PG_TEST *pgTest = new PG_TEST();
  PG_CAL_TARGET *pgCalTarget = new PG_CAL_TARGET();
  PLL_CFG *pllCfg = new PLL_CFG();
  PLL_CC *pllCc = new PLL_CC();
  PLL_CAL *pllCal = new PLL_CAL();
  XTAL *xtal = new XTAL();
  AON_DIG_CFG *aonDigCfg = new AON_DIG_CFG();
  AON_CTRL *aonCtrl = new AON_CTRL();
  AON_RDATA *aonRdata = new AON_RDATA();
  AON_ADDR *aonAddr = new AON_ADDR();
  AON_WDATA *aonWdata = new AON_WDATA();
  AON_CFG *aonCfg = new AON_CFG();
  OTP_WDATA *otpWdata = new OTP_WDATA();
  OTP_ADDR *otpAddr = new OTP_ADDR();
  OTP_CFG *otpCfg = new OTP_CFG();
  OTP_STAT *otpStat = new OTP_STAT();
  OTP_RDATA *otpRdata = new OTP_RDATA();
  OTP_SRDATA *otpSrdata = new OTP_SRDATA();
  IP_TS *ipTs = new IP_TS();
  STS_TS *stsTs = new STS_TS();
  STS1_TS *sts1Ts = new STS1_TS();
  TDOA *tdoa = new TDOA();
  PDOA *pdoa = new PDOA();
  CIA_DIAG_0 *ciaDiag0 = new CIA_DIAG_0();
  CIA_DIAG_1 *ciaDiag1 = new CIA_DIAG_1();
  IP_DIAG_0 *ip_diag0 = new IP_DIAG_0();
  IP_DIAG_1 *ipDiag1 = new IP_DIAG_1();
  IP_DIAG_2 *ipDiag2 = new IP_DIAG_2();
  IP_DIAG_3 *ipDiag3 = new IP_DIAG_3();
  IP_DIAG_4 *ipDiag4 = new IP_DIAG_4();
  IP_DIAG_8 *ipDiag8 = new IP_DIAG_8();
  IP_DIAG_12 *ipDiag12 = new IP_DIAG_12();
  STS_DIAG_0 *stsDiag0 = new STS_DIAG_0();
  STS_DIAG_1 *stsDiag1 = new STS_DIAG_1();
  STS_DIAG_2 *stsDiag2 = new STS_DIAG_2();
  STS_DIAG_3 *stsDiag3 = new STS_DIAG_3();
  STS_DIAG_4 *stsDiag4 = new STS_DIAG_4();
  STS_DIAG_8 *stsDiag8 = new STS_DIAG_8();
  STS_DIAG_12 *stsDiag12 = new STS_DIAG_12();
  STS1_DIAG_0 *sts1Diag0 = new STS1_DIAG_0();
  STS1_DIAG_1 *sts1Diag1 = new STS1_DIAG_1();
  STS1_DIAG_2 *sts1Diag2 = new STS1_DIAG_2();
  STS1_DIAG_3 *sts1Diag3 = new STS1_DIAG_3();
  STS1_DIAG_4 *sts1Diag4 = new STS1_DIAG_4();
  STS1_DIAG_8 *sts1Diag8 = new STS1_DIAG_8();
  STS1_DIAG_12 *sts1Diag12 = new STS1_DIAG_12();
  CIA_CONF *ciaConf = new CIA_CONF();
  FP_CONF *fpConf = new FP_CONF();
  IP_CONF *ipConf = new IP_CONF();
  STS_CONF_0 *stsConf0 = new STS_CONF_0();
  STS_CONF_1 *stsConf1 = new STS_CONF_1();
  CIA_ADJUST *ciaAdjust = new CIA_ADJUST();
